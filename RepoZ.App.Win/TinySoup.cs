...
namespace TinySoup
{
    ...
    public class WebSoupClient : ISoupClient
    {
        ...
        {
            return CheckForUpdatesAsync(request, new DefaultVersionComparer());
        }
        ...
        public async Task<IList<AvailableVersion>> CheckForUpdatesAsync(UpdateRequest request, IComparer<Version> versionComparer)
        {
            var parameters = new ServiceParameterCollection
                {
                    { "cid", Uri.EscapeDataString(request.ClientIdentifier?.ToString() ?? string.Empty) },
                    { "pid", Uri.EscapeDataString(request.ApplicationIdentifier ?? string.Empty) },
                    ...
                    { "vai", Uri.EscapeDataString(request.Channel ?? string.Empty) },
                    { "ext", Uri.EscapeDataString(request.PlatformIdentifier?.ToString() ?? string.Empty) },
                    { "vol", Uri.EscapeDataString(request.FreeText ?? string.Empty) },
                };

            IList<AvailableVersion> versions = await PutAsync(parameters.ToString());
        ...
        {
            ServiceParameterCollection parameters = new ServiceParameterCollection("table", table).Add("where", where);
...
namespace TinySoup.Identifier
{
    ...
    public class AnonymousClientIdentifier : IClientIdentifier
    {
        ...
        protected virtual string GetPlatformConfigPath()
        {
            switch (Environment.OSVersion.Platform)
            {
                ...
                case PlatformID.WinCE:
                    ...
                    return appDataPath ??
                           throw new PlatformNotSupportedException(nameof(AnonymousClientIdentifier) +
                                                                   " cannot be used with UWP. Derive a custom identifer from it and handle the file IO in UWP manner.");
        ...
        protected virtual string ReadId(string file)
        {
            return File.ReadAllText(file, Encoding.UTF8);
        }

        protected virtual void WriteId(string file, string id)
        {
...
namespace TinySoup.Internal
{
    ...
    internal class ServiceParameterCollection : List<ServiceParameter>
    {
        ...
        {
            var result = new ServiceParameterCollection();
...
namespace TinySoup.Model
{
    ...
    public class AvailableVersion
    {
        ...
        [DataMember(Name = "Application")] public string ApplicationIdentifier { get; set; } = string.Empty;

        [IgnoreDataMember] public Version Version => _version ?? new Version();
        ...
        public string ShortestVersionString
        {
            get
            {
                return Version.Major.ToString()
                       ...
                       + (Version.Build > 0 || Version.Revision > 0 ? "." + Version.Build.ToString() : string.Empty)
        ...
        [DataMember(Name = "ExternalInfo")] public string Info { get; set; } = string.Empty;
    ...
    public class UpdateRequest
    {
        ...
        public UpdateRequest WithNameAndVersionFromEntryAssembly()
        {
            return WithNameAndVersionFromAssembly(Assembly.GetEntryAssembly());
        }

        public UpdateRequest WithNameAndVersionFromExecutingAssembly()
        {
            return WithNameAndVersionFromAssembly(Assembly.GetExecutingAssembly());
        }

        public UpdateRequest WithNameAndVersionFromCallingAssembly()
        {
            return WithNameAndVersionFromAssembly(Assembly.GetCallingAssembly());
        }
        ...
        {
            return WithClientIdentifier(new AnonymousClientIdentifier());
        }
