...
namespace grr
{
	...
	partial class CommandLineOptions
	{
		...
		[Option(ListCommand, HelpText = "(Default) Lists the repositories found by RepoZ including their current branch and the corresponding status. Can be omitted like shown in the examples below.")]
		public ListOptions ListOptions { get; set; }

		[Option(ChangeDirectoryCommand, HelpText = "Causes the command line interface to navigate to the directory of a given repository.")]
		public ChangeDirectoryOptions ChangeDirectoryOptions { get; set; }

		[Option(OpenDirectoryCommand, HelpText = "Opens the directory or a file of a given repository with the operating system's default application.")]
		public OpenDirectoryOptions OpenDirectoryOptions { get; set; }
